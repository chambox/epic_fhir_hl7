...
class FhirEncounter(Model):
    ...
    def _extract_hospital_and_departments(self):
        ...
        stay_data = {
            "id": self.id,
            "version": None,
            "patient": self.patient if self.patient is not None else self._extract_patient(),
        ...
        for location in encounter_locations:
            ...
            if loc.is_department() and not loc.id in self.department_stays and 'period' in location:
                self.department_stays[loc.id] = {
                    "fhir_location": loc,
                    "hospital_stay": self.id,
                }

                # Check the partOf property of hos location if it is a hospital (this overrides any previously deduced hospital)
        ...
        for bed_id in self.beds:
            ...
            if room_id:
                if room_id in self.rooms:
                    self.rooms[room_id]["bed"] = self.beds[bed_id]["fhir_location"].get_reference_object()
        ...
        for room_id in self.rooms:
            ...
            if department_id:
                if department_id in self.department_stays:
                    self.department_stays[department_id]["room"] = self.rooms[room_id]["fhir_location"].get_reference_object(
                    )
    ...
    def _match_departments_to_hospitals(self):
        for department_id in self.department_stays:
            ...
            if hospital_id in self.hospitals:
                hospital_reference = self.hospitals[hospital_id]['fhir_location'].get_reference_object()
                if not hospital_id in self.hospital_stays:
                    # create hospital_stay object for this hospital if it does not already exist
                    self.hospital_stays[hospital_id] = self.hospitals[hospital_id]['stay_data']
                ...
                self.hospital_stays[hospital_id]["department_stays"].append({
                    ...
                    "location": {
                        ...
                        "department": self.department_stays[department_id]['fhir_location'].get_reference_object(),
                        "room": self.department_stays[department_id].get("room", None),
                        "bed": self.department_stays[department_id].get("room", {}).get("bed")
                    },
                    "starts_at": self.department_stays[department_id]['fhir_location'].period.get("start", None),
                })
